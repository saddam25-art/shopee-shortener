...
<body class="bg">
    ...
    <div id="mainSection" class="hide">
      <div class="grid">
        <div><div class="card">
          ...
          <p class="muted" style="margin:6px 0 0;">Image will be resized to 940×788 px</p>
  ...
  <script>
    ...
    ogFileEl.addEventListener('change', async (e) => { const file = e.target.files[0]; if (!file) return; formStatusEl.textContent = 'Resizing...'; const resized = await resizeImage(file, 940, 788); formStatusEl.textContent = 'Uploading...'; const fd = new FormData(); fd.append('file', resized, file.name.replace(/\\.[^.]+$/, '.png')); const res = await fetch(BASE + '/api/uploads', { method: 'POST', credentials: 'include', body: fd }); const data = await res.json(); if (!res.ok) { formStatusEl.textContent = data.error || 'Upload failed'; return; } ogImageUrlEl.value = data.url; ogPreviewEl.src = data.url; ogPreviewEl.classList.remove('hide'); formStatusEl.textContent = 'Uploaded!'; });
